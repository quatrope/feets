...
class FeatureSpace:
                        ...
                        if inspect.isclass(obj) and name != 'Base' and not name in excludeList:
                            if name in kwargs.keys():
                                ...
                                if set(obj().Data).issubset(self.Data):
                                    ...
                                    msg = "the feature {} could not be calculated because {} are needed.".format(name, obj().Data)
                                    warnings.warn(msg, FeatureWarning)
        ...
        for item in self.featureList:
            if item in kwargs.keys():
                try:
                    ...
                    raise FeaturError(item)
            else:
                try:
                    a = getattr(m, item)()
                except:
                    raise FeatureNotFound(item)
            try:
                self.featureFunc.append(a.fit)
            except:
                msg = "could not initilize " + item
                warnings.warn(msg, FeatureWarning)
    ...
    def _make_result(self, result):
        if self.sort == True:
            fnames, fvalues = [], []
            for idx in self.idx:
                fnames.append(self.featureList[idx])
                fvalues.append(result[idx])
        else:
            fnames = self.featureList
            fvalues = result
        return np.asarray(fnames), np.asarray(fvalues)

    def calculateFeature(self, data):
        X, result = np.asarray(data), []
        for f in self.featureFunc:
            result.append(f(X))
